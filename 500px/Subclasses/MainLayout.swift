...
class MainLayout: UICollectionViewLayout {
    ...
    override func prepare() {
        ...
        columnOffsets = [CGFloat].init(repeating: 0, count: numberOfColumns)
    ...
    private func setColumnWidth() {
        self.columnWidth = round(self.collectionView!.bounds.size.width / CGFloat(self.numberOfColumns))
    }
    
    private func canDoubleCellWidth(at index: Int, indexPath: IndexPath) -> Bool {
        return (index < self.numberOfColumns - 1) && self.columnOffsets[index] == self.columnOffsets[index+1] && indexPath.item % 10 < doubleCellProbability
    }
}
